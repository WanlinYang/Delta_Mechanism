...
displacement = 0.001;           % displacement distance(m) in stiffness calculation
stiffness_scale = 0.00005;       % plot scale of ellipse
...
for i=1:3
    ...
    hold on
    if i==1
        title('Force limits','FontWeight', 'bold', 'Color', 'k')
    end
    if i==2
        title('Velocity limits','FontWeight', 'bold', 'Color', 'k')
    end
    if i==3
        title('Stiffness ellipse','FontWeight', 'bold', 'Color', 'k')
    end
    view(90,0);
end
...
%% plot stiffness ellipse at representative points
subplot(1,3,3)
% y max
DeltaStiffnessEllipse( R1,R2,L1,L2,p_y_max,K,theta_home,displacement,stiffness_scale );
hold on

% y min
DeltaStiffnessEllipse( R1,R2,L1,L2,p_y_min,K,theta_home,displacement,stiffness_scale );
hold on

% z max
DeltaStiffnessEllipse( R1,R2,L1,L2,p_z_max,K,theta_home,displacement,stiffness_scale );
hold on

% z min
DeltaStiffnessEllipse( R1,R2,L1,L2,p_z_min,K,theta_home,displacement,stiffness_scale );
